...
- On phones and short landscape screens, card faces are the action controls: market draws, hand selects (or deselects), and your creature rotates or receives the selected knowledge. The separate ⓘ button opens details without making a move. Empty knowledge placeholders and separate Draw/Rotate/Select buttons are removed in this layout; occupied knowledge stays visible. The tutorial describes the matching touch gesture. Desktop keeps its explicit buttons.
- Card inspection is separate from gameplay. Invalid plays preserve selection. Replacing an occupied slot identifies the outgoing card on the creature before the tap. Direct action and inspection controls remain keyboard accessible, with separate touch targets. Resizing changes the controls without replacing the match state.
