...
const CardDetailOverlay: React.FC<CardDetailOverlayProps> = ({ card, open, onClose, contextLabel, showBack = false }) => {
  ...
  return createPortal(
    ...
    >
      <div
        className="grid w-full max-w-3xl gap-4 rounded-2xl border border-white/12 bg-[#080d19] p-4 text-white shadow-[0_28px_90px_rgba(0,0,0,0.72)] sm:grid-cols-[minmax(180px,260px)_1fr] sm:p-5"
        onMouseDown={(event) => event.stopPropagation()}
      >
        <div className="mx-auto aspect-[921/1217] w-full max-w-[220px] overflow-hidden rounded-[14px] border border-amber-200/40 bg-slate-950 shadow-[0_18px_44px_rgba(0,0,0,0.48)] sm:max-w-none">
        ...
        <div className="flex min-w-0 flex-col">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              {contextLabel && <p className="text-xs font-bold uppercase tracking-widest text-cyan-200">{contextLabel}</p>}
            ...
            <button
              type="button"
              className="grid h-10 w-10 shrink-0 place-items-center rounded-xl border border-white/10 bg-white/[0.04] text-slate-200 transition hover:bg-white/[0.08] focus:outline-none focus:ring-2 focus:ring-amber-300/40"
              onClick={onClose}
          ...
          <div className={cn('mt-5 rounded-2xl border border-white/10 bg-white/[0.04] p-4', showBack ? 'text-slate-400' : 'text-slate-200')}>
