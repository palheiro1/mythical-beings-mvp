...
var CreatureZone = function (_a) {
    var creatures = _a.creatures, field = _a.field, onCreatureClick = _a.onCreatureClick;
    return (
    ...
    <div className="flex flex-wrap justify-center md:justify-around items-start p-2 md:p-4 bg-green-900 rounded min-h-[150px] md:min-h-[200px] gap-1 md:gap-2">
      ...
      {creatures.map(function (creature) {
            var fieldSlot = field.find(function (f) { return f.creatureId === creature.id; });
            var attachedKnowledge = fieldSlot === null || fieldSlot === void 0 ? void 0 : fieldSlot.knowledge;
            return (
            ...
            <div className="w-24 md:w-28 h-32 md:h-40 relative">
              ...
              <div className="absolute inset-0 z-10">
                <Card_1["default"] card={creature} onClick={onCreatureClick ? function () { return onCreatureClick(creature.id); } : undefined}/>
              </div>
              
              {/* Attached Knowledge Card (if any) - positioned on top with rotation */}
              {attachedKnowledge && (<div className={"\n                  absolute inset-0 transform translate-y-6 md:translate-y-8\n                  ".concat(attachedKnowledge.rotation ? 'z-30' : 'z-20', " // Increase z-index when rotated\n                ")}>
                  <Card_1["default"] card={attachedKnowledge} rotation={attachedKnowledge.rotation || 0}/>
                </div>)}
            </div>
          </div>);
        })}
