...
function PracticeMatch({ team, mode, gameId, onReplay }: { team: string[]; mode: TrainingMode; gameId: string; onReplay: () => void }) {
  ...
  return <div ref={containerRef} className={`wd wd-match ${paused ? 'has-guide' : ''}`}>
    ...
    {pending?.playerId === HUMAN_ID && !inspection && <TrainingDialog title={pending.sourceKnowledgeName || 'Choose a target'}><p>{pending.prompt}</p><div className="wd-effect-choices">{pending.choices.map((choice, index) => <button key={index} className="wd-effect-choice" onClick={() => onAction({ type: 'RESOLVE_PENDING_EFFECT', payload: { playerId: HUMAN_ID, resolution: { effectId: pending.id, choice } } })}>{choice.image && <CardArtwork src={choice.image} alt="" className="wd-artwork" sizes="160px" />}<span>{choice.label}</span></button>)}</div>{pending.optional && <button className="wd-button" onClick={() => onAction({ type: 'RESOLVE_PENDING_EFFECT', payload: { playerId: HUMAN_ID, resolution: { effectId: pending.id, skip: true } } })}>Skip effect</button>}</TrainingDialog>}
